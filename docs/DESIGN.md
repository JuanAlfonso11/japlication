...
- Swipe derecha (o botón ✓) = guardar/aplicar → pasa a `applications` con estado `saved`, con animación de vuelo hacia la derecha + sello "SAVE" + lavado de color verde sobre la tarjeta (`frontend/components/SwipeCard.tsx`, Framer Motion).
- Swipe izquierda (o botón ✕) = descartar → estado `passed`, no vuelve a aparecer en la cola — pero la vacante en sí **no se bloquea ni se elimina**: sigue existiendo normalmente y reaparece si el usuario la busca de nuevo en Discover; "passed" solo la excluye de la cola de Home. Misma animación que "guardar" mas en espejo (vuelo a la izquierda + sello "PASS" + lavado rosa). Los botones ✓/✕ y las flechas del teclado disparan la misma animación que un swipe real (antes solo el gesto de arrastre la mostraba); la mutación real (llamada a la API + remoción de la cola) se dispara al terminar la animación, no antes, para que el vuelo se vea completo sin importar qué tan rápido responda el backend.
...
- **Búsqueda en vivo**: 15 proveedores (12 sin autenticación, 3 con clave propia) detrás de un único endpoint agregado (`GET /jobs/search/aggregate`, con `GET /jobs/search?provider=` disponible para consultar uno solo). Los resultados normalizados se guardan para que "agregar a mi cola" no dispare una segunda consulta al proveedor: en memoria del proceso como camino rápido, y en `external_job_cache` detrás, porque los diccionarios en memoria se vacían con cada reinicio y el usuario, que tiene la lista en pantalla, no tiene por qué enterarse de eso.
...
- **Match Engine**: fórmula híbrida ponderada (ver detalle técnico en `backend/app/services/match_engine.py`): 50% técnico (overlap de skills), 30% experiencia (años requeridos vs. acumulados relevantes), 20% semántico (similitud texto perfil↔vacante, con fallback TF‑IDF sin dependencia de API externa).
...
- **Móvil**: la interfaz web es responsive y se sirve como PWA instalable (manifest + iconos) — swipe funciona igual de bien vía gestos táctiles en el navegador móvil. Además hay un shell nativo Android opcional (`frontend/android/`, vía Capacitor) que carga ese mismo frontend en vivo dentro de un WebView con ícono propio — no empaqueta una copia de la app, así que la mayoría de cambios de frontend no requieren recompilar el `.apk`. Para que funcione desde cualquier lugar (no solo en casa) sin exponer la red del usuario a internet, apunta al hostname MagicDNS de **Tailscale** de la PC (VPN privada tipo mesh entre los propios dispositivos, sin tocar el router) en vez de a un puerto abierto en el router — ver `docs/ANDROID_APP.md`.
- **Tema claro/oscuro**: selector de tres estados (Claro/Oscuro/Sistema) en el header, vía `frontend/context/ThemeContext.tsx` — "Sistema" (default) sigue `prefers-color-scheme` del SO en tiempo real; una elección explícita se guarda en `localStorage` y se aplica como clase `dark` en `<html>` (Tailwind `darkMode: "class"`). Un script inline en `<head>` (antes de hidratar React) aplica la clase correcta desde el primer paint para evitar el parpadeo de tema equivocado.
