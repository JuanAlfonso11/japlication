 ...
 * shows up in the /swipe queue right away instead of silently sitting
 * unmatched until someone happens to open it. */
async function importAndMatch(job: Job): Promise<Job> {
  try {
...
function UpworkConnectPanel({ status, onConnected }: { status: UpworkStatus; onConnected: () => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      {provider === "upwork" && upworkStatus && !upworkStatus.connected && (
        <UpworkConnectPanel status={upworkStatus} onConnected={refreshUpworkStatus} />
      )}
        ...
        <>
          <form onSubmit={handleSubmit} className="flex flex-wrap gap-2">
            <input
              ...
              placeholder={provider === "upwork" ? "Skills or title, e.g. Django, React" : "Job title or keywords"}
    ...
    <div className="space-y-6 pb-4 animate-fade-in">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">
          Search live listings, paste a posting URL, or add one by hand — JobFlow AI parses it into a
          structured job and matches it against your profile.
        </p>
      </div>
      ...
      <div className="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
          ...
          {tabs.map((t) => (
            <button
              ...
              className={`rounded-md px-3 py-1.5 transition-colors ${
                mode === t.id ? "bg-white shadow-sm text-gray-900" : "text-gray-500"
              }`}
            >
              {t.label}
            </button>
        ...
        {mode === "url" && <ImportByUrl onImported={setLastImported} />}
        {mode === "manual" && <ManualJobForm onImported={setLastImported} />}
      </div>
