...
| `apk-server/serve-apk.ps1` | Always-on static server on `http://localhost:8446` that hands out `apk-server/jobpilot.apk` — what the in-app update banner downloads. `tailscale funnel` maps `https://<host>.ts.net:10000` to it (public). Runs outside Docker (like the offline page), so an update stays downloadable even while the containers are stopped. No authentication: anyone on the internet can fetch it, which is fine because the APK is a WebView shell with no secrets in it (every credential lives behind the backend login). |
| `install-apk-server-autostart.ps1` | Run once: reserves `http://+:8446/` (one-time UAC prompt — needed so the listener accepts the real Host header `tailscale funnel` forwards), registers the server to start at login, and adds the `--https=10000` funnel route. Already installed. |
